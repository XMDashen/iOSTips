...
//        _pageControl.tintColor = [DDSkin DDThemeRedColor];
        _pageControl.frame = CGRectMake(0, 0, kHcdGuideViewBounds.size.width, 44.0f);
        _pageControl.center = CGPointMake(kHcdGuideViewBounds.size.width / 2, kHcdGuideViewBounds.size.height - 25);
    }
    return _pageControl;
}
...
- (void)nextButtonHandler:(id)sender {
    ...
    [NOTIFICATION_CENTER postNotificationName:@"LocationNotification" object:nil];
    
    //第一次进入app时，如果没有定位， 不会弹出提示框，以后在app进入前台的时候如果没有定位就会弹出提示框，让用户开启定位 (applicationDidBecomeActive)
    [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"NotFirstInApp"];
